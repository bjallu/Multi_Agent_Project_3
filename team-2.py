...
class DecisionTreeNode :
  ...
  def get_distances_to_food_factor(self,agent_position,agent_index,enemy_team):
    ...
    distance_to_food_factor = distance_to_food_factor * 5
  ...
  def kill_enemy_agent_factor(self,agent_positions,agent_index):
    ...
    if we_are_blue_team:
      ...
      if (agent_index < 2):
        goal_agent = (self.node_state.data.layout.width/2 + 1,self.node_state.data.layout.height/4)
      else:
        goal_agent = (self.node_state.data.layout.width/2 + 1, 3 * self.node_state.data.layout.height / 4)
    else:
